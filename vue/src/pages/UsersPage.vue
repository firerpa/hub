...
type UserSortField = "id" | "name" | "contact" | "register_time" | "last_login_time" | "is_admin" | "last_login_ip";
type UserItem = {
  id: number;
  name: string;
  contact?: string;
...
const sortField = ref<UserSortField>("id");
...
const filterQuery = computed(() => {
  if (!deferredSearch.value.trim()) return undefined;
  return JSON.stringify([{ field: "name", op: "like", value: deferredSearch.value.trim() }]);
});
...
onMounted(async () => {
  try {
    ...
    selfId.value = Number(res?.data?.id ?? 0);
  } catch {}
});
onUnmounted(() => {
  selfCancelled = true;
    ...
    const load = async () => {
      ...
      try {
        ...
        const mapped: UserItem[] = list.map((u: any) => ({
          ...
          register_time: Number(u?.register_time ?? 0),
        ...
        showTip(formatApiError(error, t.value.usersPage.loadFailed));
      } finally {
...
const columns = computed<Array<{ field?: UserSortField; label: string }>>(() => [
  { field: "name", label: t.value.usersPage.colName },
  { field: "contact", label: t.value.usersPage.colContact },
  { field: "last_login_ip", label: t.value.usersPage.colLoginIp },
  { field: "register_time", label: t.value.usersPage.colRegisterTime },
  { field: "last_login_time", label: t.value.usersPage.colLoginTime },
  { field: "is_admin", label: t.value.usersPage.colAdmin },
]);

const handleBatchDelete = async () => {
  const ids = Array.from(selected.value).filter((id) => id !== selfId.value);
  if (!ids.length) return;
  loading.value = true;
  try {
    const results = await Promise.allSettled(ids.map((id) => apiRequest(`/api/v1/user/${id}`, { method: "DELETE" })));
    const ok = results.filter((r) => r.status === "fulfilled").length;
    selected.value = new Set();
    if (ok === ids.length) showTip(t.value.usersPage.deleteSuccess.replace("{n}", String(ok)), "success");
    else showTip(t.value.usersPage.deleteFailed);
  } catch (error) {
    showTip(formatApiError(error, t.value.usersPage.deleteFailed));
  } finally {
    refreshTick.value += 1;
    loading.value = false;
  }
...
const onPanelSuccess = () => {
  createOpen.value = false;
  refreshTick.value += 1;
  showTip(t.value.usersPage.createSuccess, "success");
};
</script>
    ...
    <div class="flex min-w-0 flex-1 flex-col overflow-hidden lg:ml-[220px]">
      <div class="h-14 shrink-0 border-b border-gray-100 bg-white px-5 py-2.5">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-base font-semibold">{{ t.usersPage.title }}</h1>
          </div>
      ...
      <div class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
        <div class="shrink-0 border-b border-gray-100 px-4 py-3">
          <div class="flex flex-wrap items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <h2 class="text-sm font-semibold text-gray-800">{{ t.usersPage.allUsers }}</h2>
            ...
            <tbody>
              ...
              <tr v-else-if="rows.length === 0">
                <td :colspan="7" class="py-20 text-center text-muted-foreground">
                  <EmptyState
                    :title="t.usersPage.noData"
                    :description="search ? t.usersPage.noDataDescFiltered : t.usersPage.noDataDescDefault"
                  />
                </td>
              </tr>
              <template v-else>
                <tr
                  ...
                  :class="
                    cn(
                      ...
                      selected.has(u.id) && 'bg-gray-100/70',
                    )
                  "
                >
                  <td class="w-10 px-3 py-2">
                    <Checkbox :checked="selected.has(u.id)" @update:checked="(c) => onToggleOne(u.id, c === true)" />
                  </td>
                  <td class="px-3 py-2 font-medium">{{ u.name }}</td>
                  <td class="px-3 py-2">{{ u.contact || "-" }}</td>
                  <td class="px-3 py-2">{{ u.last_login_ip || "-" }}</td>
                  <td class="whitespace-nowrap px-3 py-2 text-muted-foreground">{{ tsText(u.register_time) }}</td>
                  <td class="whitespace-nowrap px-3 py-2 text-muted-foreground">{{ tsText(u.last_login_time) }}</td>
                  <td class="px-3 py-2">
                    <span
                      :class="
                        cn(
                      ...
                      "
                    >
                      {{ u.is_admin ? t.usersPage.adminYes : t.usersPage.adminNo }}
                    </span>
                  </td>
                </tr>
              </template>
        ...
        <div class="shrink-0 border-t border-gray-100 bg-white px-4 py-2">
          <div class="flex items-center justify-between">
            <span class="text-xs text-muted-foreground">
              {{ t.usersPage.total.replace("{total}", String(total)) }}
            </span>
            <div class="flex items-center gap-2">
              <Select
      ...
      >
        <span class="text-xs font-medium text-foreground">{{ selected.size }} {{ t.usersPage.itemsSelected }}</span>
    ...
    <Sheet :open="createOpen" @update:open="createOpen = $event">
      <SheetContent side="right" class="w-full gap-0 p-0 sm:max-w-[520px]">
        <SheetHeader class="border-b border-border pr-12">
          <SheetTitle>{{ t.usersPage.createTitle }}</SheetTitle>
        </SheetHeader>
        <CreateUserPanel compact :on-cancel="onPanelCancel" :on-success="onPanelSuccess" />
      </SheetContent>
    </Sheet>
  </div>
</template>
